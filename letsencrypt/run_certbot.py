...
def run_hook(hook_file, *args):
    if os.path.exists(hook_file):
        os.chmod(hook_file, hook_file.st_mode | stat.S_IEXEC | stat.S_IXGRP | stat.S_IXOTH)
        subprocess.call(["hook_file"].extends(args), stderr=PIPE, universal_newlines=True)
...
def main():
    ...
    run_hook("/etc/letsencrypt/hooks/pre-run.hook")
        ...
        for domain in ocf["domains"]:
            if os.path.exists("/etc/letsencrypt/live/{0}/cert.pem".format(domain)):
                ...
                if days_left < days_before_expiry:
                    run_hook("/etc/letsencrypt/hooks/pre-create.hook", str(domain))
                    ...
                    if p.returncode == 0:
                        ...
                        run_hook("/etc/letsencrypt/hooks/post-create.hook", str(domain))
                    else:
