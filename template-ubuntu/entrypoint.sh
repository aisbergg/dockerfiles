...
function toUpper() {
    ...
    # Usage: string=`toUpper "$string"`
    echo "$1" | tr '[:lower:]' '[:upper:]'
}

function toLower() {
    # Converts a string to lowercase.
    # Usage: string=`toLower "$string"`
    echo "$1" | tr '[:upper:]' '[:lower:]'
}

function standardiseBool() {
    ...
    # Usage: bool=`standardiseBool "$bool" True`
    local bool=`toLower "$1"`
...
function exportBool() {
    ...
    for (( i=0; i < ${#bools[@]}; i=i+2 )); do
        eval export "${bools[$i]}=\"`standardiseBool "${!bools[$i]}" "${bools[$i+1]}" `\""
    done
}
...
function isDirEmpty() {
    ...
    # Usage: if isDirEmpty /some/dir; then ...; fi
    if [ -e "$1" ]; then
...
function mergeDirs() {
    ...
    # Usage: mergeDirs /merge/from /into/dest/dir
    local src="$1"
    local dest="$2"
...
function copyFileIfAutogen() {
    ...
    # Usage: copyFileIfAutogen /src/dir /dest/dir

    if [ -f "$2" ]; then
        local header="$(head -n 1 "$2")"
        if [[ "$header" =~ autogen[[:blank:]]+([^[:blank:]]+) ]]; then
            local autogen=`standardiseBool "${BASH_REMATCH[1]}" True`
        fi
    fi
...
if [[ ! "$debug_mode" == True || "$provision" == True ]]; then
    if [ -d /provision ]; then
        ...
        for rel_path in ${relative_file_paths[@]}; do
            copyFileIfAutogen "/provision/templates/${rel_path}" "/${rel_path}"
        done
